      subroutine VManomalyBLH(BLH,gra,hgt,nlat,nlon,hd,dr,vm,GRS)
      !按严密球面积分公式计算广义V_M空间异常积分
      !dr-积分半径m
!-------------------------------------------------------------
      implicit none
	  ...
	  do j=j0-nj,j0+nj
	    ...
	    BLH1(2)=hd(1)+(real(j)-0.5d0)*hd(5)
          ...
          if(L0<mdr)then!计算奇异积分
             call VManomSgn(BLH,gra,hgt,nlat,nlon,hd,rst,i,j,4,GRS)
	    ...
	    sina=dcos(rlat1)*dsin(rlon1-rlon)/sinf
          SK=-2.d0*rr/L1**3-3.d0/rr/L1+5.d0/rr**2+3.d0/rr**2*dlog((rr+L1-r1*tt)/2.d0/rr)
          SK=SK-3.d0*r1*(L1+rr)*tt/rr**2/L1/(rr+L1-r1*tt);SK=SK*r1*sinf
...
!--------------------------------------------------------------------------------
      subroutine VManomSgn(BLH,gra,hgt,nlat,nlon,hd,vm,i0,j0,m,GRS)
      !细化核函数，计算BLH点的广义V_M空间异常奇异积分
      !m-核函数细化为m*m
      !i0,j0-奇异点格网位置
!-------------------------------------------------------------
      implicit none
